...
import com.example.lmmovilmantenimiento.Preventivos.ViewHolderPreventivos.AdapterListadoSucursalePreventivos;
...
public class MuestreoSucursales extends AppCompatActivity {
    private DatabaseReference referenciaCargarInicial;
    private FirebaseDatabase myDb;
    private RecyclerView rvListadoErroresPDF;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_muestreo_sucursales);
    ...
    private void listeners() {
        ibRegresar.setOnClickListener(view ->{
            Intent intent = new Intent(MuestreoSucursales.this, InicioActivity.class);
    ...
    private void validarUsuario() {
        ...
        referenciaUsuario.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    if (Objects.requireNonNull(snapshot1.child("uid").getValue()).toString().equals(Objects.requireNonNull(auth.getCurrentUser()).getUid())){
                        ...
                        referenciaPerfifles.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                for (DataSnapshot snapshot2 : snapshot.getChildren()){
                                    if (Objects.requireNonNull(snapshot1.child("id_perfil").getValue()).toString().equals(snapshot2.getKey())){
                                        if (Objects.requireNonNull(snapshot2.child("descripcion").getValue()).toString().equals("gerente")){
                                            Intent intent = new Intent(MuestreoSucursales.this, ListadoPreventivos.class);
                            ...
                            public void onCancelled(@NonNull DatabaseError error) {
                                Toasty.error(MuestreoSucursales.this, "Error: " + error.getCode(), Toasty.LENGTH_SHORT).show();
                            }
                        });
                    }
                }
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toasty.warning(MuestreoSucursales.this, "Error: "+error.getCode(), Toasty.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void cargarRv() {
        ...
        mLayoutManager = new LinearLayoutManager(MuestreoSucursales.this);
        rvListadoErroresPDF.setLayoutManager(mLayoutManager);
        layout = findViewById(R.id.consLayout);
        mAdapter = new AdapterListadoSucursalePreventivos(obtenerDataHistorial(),MuestreoSucursales.this);
